...
/* 会话正文内的命中高亮(两段式: 命中瞬间亮色 → 1.4s 后转常驻淡色) */
::highlight(cnvnav-find-hot) {
  background: color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f59e0b) 55%, transparent);
  color: inherit;
}
::highlight(cnvnav-find) {
  background: color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f59e0b) 22%, transparent);
  color: inherit;
}
...
`;
		...
		function previewFor(node) {
			...
			const FIND_HOT = "cnvnav-find-hot"; /* 命中瞬间的亮色层 */
			const FIND_DIM = "cnvnav-find";     /* 闪过后常驻的淡色层 */
			const FIND_FLASH_MS = 1400;
			...
			function clearFindHighlight() {
				try {
					...
					if (store !== null) {
						store.delete(FIND_HOT);
						store.delete(FIND_DIM);
					}
			...
			/* 正文内定位并高亮: 命中瞬间亮色, FIND_FLASH_MS 后换成常驻淡色; 返回是否成功 */
			function locateInConversation(row, kw) {
				...
				try {
					...
					store.set(FIND_HOT, new Ctor(...ranges));
					findTimer = setTimeout(() => {
						findTimer = null;
						try {
							const laterStore = highlightApi();
							if (laterStore !== null) {
								laterStore.delete(FIND_HOT);
								laterStore.set(FIND_DIM, new Ctor(...ranges));
							}
						} catch (err) { console.error(err); }
					}, FIND_FLASH_MS);
