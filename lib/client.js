...
`;
		...
		function previewFor(node) {
			...
			let lastActive = null;
						/* ---------- 会话正文内的命中定位(点击搜索命中后把关键词标在正文里) ---------- */
			const FIND_STEPS = 28;     /* 淡出用的预置层数(层名 cnvnav-find-s0 … s27) */
			const FIND_HOT_PCT = 55;   /* 命中瞬间的背景浓度(%) */
			const FIND_DIM_PCT = 22;   /* 最终常驻的淡色浓度(%) */
			const FIND_HOLD_MS = 300;  /* 最亮保持时长 */
			const FIND_FADE_MS = 900;  /* 变淡时长(easeOutCubic 逐帧换层) */
			const FIND_MAX_RANGES = 200;
			const FIND_BLOCK_TAGS = { P: 1, DIV: 1, LI: 1, UL: 1, OL: 1, PRE: 1, BLOCKQUOTE: 1, TD: 1, TH: 1, TR: 1, TABLE: 1, SECTION: 1, ARTICLE: 1, H1: 1, H2: 1, H3: 1, H4: 1, H5: 1, H6: 1 };
			let findRaf = null;    /* 淡出动画句柄 */
			let findStep = null;   /* 当前注册的层号 */
			let findMark = null;   /* 当前高亮的 Highlight 对象(各层共用同一份 Range) */
			function findLayerName(step) {
				return "cnvnav-find-s" + step;
			}
			function findAlphaPct(step) {
				const k = FIND_STEPS <= 1 ? 1 : step / (FIND_STEPS - 1);
				return FIND_HOT_PCT + (FIND_DIM_PCT - FIND_HOT_PCT) * k;
			}
			/* ::highlight 伪元素不支持 transition/animation, 只能预置多层不同浓度逐帧切换 */
			function findFadeCss() {
				let out = "\n/* 会话正文内的命中高亮: 逐帧换层实现平滑淡出(::highlight 不支持 transition) */";
				for (let i = 0; i < FIND_STEPS; i++) {
					out += "\n::highlight(" + findLayerName(i) + ") {\n  background: color-mix(in srgb, var(--dsw-alias-state-warn-primary, #f59e0b) " + findAlphaPct(i).toFixed(1) + "%, transparent);\n  color: inherit;\n}";
				}
				return out;
			}
			(function injectFindFadeCss() {
				try {
					const el = document.createElement("style");
					el.textContent = findFadeCss();
					document.head.appendChild(el);
				} catch (err) { console.error(err); }
			})();
			/* 模块顶部的 const CSS 是插件自己的样式字符串, 会遮蔽浏览器全局 CSS, 故走 globalThis */
			function highlightApi() {
				try {
					const api = typeof globalThis === "undefined" ? undefined : globalThis.CSS;
					if (api === undefined || api === null || api.highlights === undefined) return null;
					return api.highlights;
				} catch (err) { return null; }
			}
			function highlightCtor() {
				try {
					return typeof globalThis === "undefined" ? undefined : globalThis.Highlight;
				} catch (err) { return undefined; }
			}
			function dropFindLayer() {
				const store = highlightApi();
				if (store !== null && findStep !== null) store.delete(findLayerName(findStep));
				findStep = null;
			}
			function showFindStep(step) {
				const store = highlightApi();
				if (store === null || findMark === null) return;
				const next = Math.max(0, Math.min(FIND_STEPS - 1, step));
				if (next === findStep) return;
				if (findStep !== null) store.delete(findLayerName(findStep));
				store.set(findLayerName(next), findMark);
				findStep = next;
			}
			function stopFindFade() {
				if (findRaf !== null) {
					if (typeof cancelAnimationFrame === "function") cancelAnimationFrame(findRaf);
					findRaf = null;
				}
			}
			function startFindFade() {
				stopFindFade();
				if (typeof requestAnimationFrame !== "function") { showFindStep(FIND_STEPS - 1); return; }
				const now = () => (typeof performance !== "undefined" && typeof performance.now === "function" ? performance.now() : Date.now());
				const started = now();
				const tick = () => {
					const t = now() - started;
					if (t <= FIND_HOLD_MS) showFindStep(0);
					else if (t >= FIND_HOLD_MS + FIND_FADE_MS) { showFindStep(FIND_STEPS - 1); findRaf = null; return; }
					else {
						const u = (t - FIND_HOLD_MS) / FIND_FADE_MS;
						const eased = 1 - Math.pow(1 - u, 3); /* easeOutCubic */
						showFindStep(Math.round(eased * (FIND_STEPS - 1)));
					}
					findRaf = requestAnimationFrame(tick);
				};
				findRaf = requestAnimationFrame(tick);
			}
			...
			/* 最近的块级祖先: 跨块边界补一个分隔符, 避免两段文字被误判成一个词 */
			function findBlockOf(node) {
				let el = node.parentElement;
				while (el !== null && el !== undefined) {
					if (FIND_BLOCK_TAGS[el.tagName] === 1) return el;
					el = el.parentElement;
				}
				return null;
			}
			/* 收集锚点子树内可见的文本节点(跳过脚本/样式) */
			function collectFindTextNodes(root) {
				const out = [];
				const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
					acceptNode(node) {
						const text = node.nodeValue;
						if (text === null || text === "" || text.trim() === "") return NodeFilter.FILTER_REJECT;
						const parent = node.parentElement;
						if (parent === null || parent === undefined) return NodeFilter.FILTER_REJECT;
			...
			/* 归一化索引: 连续空白折叠成单个空格; map[i] = [文本节点, 节点内偏移] */
			function buildFindIndex(root) {
				const nodes = collectFindTextNodes(root);
				let text = "";
				const map = [];
				let prevBlock = null;
				...
				for (const node of nodes) {
					...
					const block = findBlockOf(node);
					if (text !== "" && (block === null || block !== prevBlock)) pendingSpace = true;
					prevBlock = block;
					for (let i = 0; i < value.length; i++) {
						const ch = value.charAt(i);
						if (ch === " " || ch === "\t" || ch === "\n" || ch === "\r" || ch === "\f" || ch === "\v") { pendingSpace = text !== ""; continue; }
						if (pendingSpace) { text += " "; map.push([node, i]); pendingSpace = false; }
						text += ch;
						map.push([node, i]);
					}
				}
			...
			function findNeedle(kw) {
				return String(kw).replace(/\s+/g, " ").trim().toLowerCase();
			}
			/* 关键词在 DOM 里的每一处出现 → Range(最多 FIND_MAX_RANGES 处) */
			function findKeywordRanges(root, needle) {
				const ranges = [];
				if (needle === "") return ranges;
				const index = buildFindIndex(root);
				if (index.text === "") return ranges;
				let hay = index.text.toLowerCase();
				if (hay.length !== index.text.length) hay = index.text; /* 极端 Unicode 大小写长度变化时退回原串 */
				let from = 0;
				for (;;) {
					...
					if (start !== undefined && end !== undefined) {
						try {
							...
							ranges.push(range);
						} catch (err) { /* 节点已被宿主重渲染替换: 跳过这一处 */ }
					}
					from = at + needle.length;
					if (ranges.length >= FIND_MAX_RANGES) break;
				}
				return ranges;
			}
			/* 把首个命中滚到视口偏上处(而不是消息顶部) */
			function scrollRangeIntoView(range) {
				try {
					...
					if (scrollport !== null && scrollport !== undefined) {
						...
						const delta = rect.top - box.top - Math.max(72, box.height * 0.32);
						if (typeof scrollport.scrollBy === "function") scrollport.scrollBy({ top: delta, behavior: "smooth" });
						else scrollport.scrollTop += delta;
						return;
					}
					const host = range.startContainer;
					const el = host.nodeType === 1 ? host : host.parentElement;
					if (el !== null && el !== undefined && typeof el.scrollIntoView === "function") el.scrollIntoView({ behavior: "smooth", block: "center" });
				} catch (err) { console.error(err); }
			}
			/* 正文内定位并高亮: 先最亮, 再 easeOut 逐帧变淡到常驻淡色; 返回是否成功 */
			function locateInConversation(row, kw) {
				clearFindHighlight();
				try {
					const store = highlightApi();
					const Ctor = highlightCtor();
					if (store === null || typeof Ctor !== "function") return false;
					const needle = findNeedle(kw);
					const ranges = findKeywordRanges(row, needle);
					if (ranges.length === 0) return false;
					findMark = new Ctor(...ranges);
					showFindStep(0);
					startFindFade();
			...
			function jumpTo(key, kw) {
				try {
					const el = findAnchor(key);
					if (el === null) return false;
					lastActive = key;
					setState({ activeKey: key });
					/* 搜索命中: 直接定位到关键词所在处并在正文里高亮; 其余情况退回消息顶部 */
					if (typeof kw === "string" && kw !== "" && locateInConversation(el, kw)) return true;
					clearFindHighlight();
			...
			function Panel() {
				...
				function renderItem(item, staggerMs) {
					...
					return React.createElement("button", {
						...
						onClick: () => jumpTo(item.key, filtering && hit(item.search) ? kw : null),
					},
				...
				function renderGroupHead(g, groupActive, chevron) {
					...
					/* 组头自身命中时, 点击同样在正文里定位关键词 */
					const headKw = filtering && g.headIsUser && g.items.length > 0 && hit(g.items[0].search) ? kw : null;
					/* 标题策略:
						...
						React.createElement(Tooltip, {
							...
							React.createElement("span", { className: "cnvnav-group-head-wrap" },
								React.createElement("button", {
									...
									onClick: () => jumpTo(g.headKey, headKw),
								},
									titleNode,
									hasSub
					...
					React.createElement("div", {
						...
						searchOpen
							? React.createElement("input", {
								...
								value: filter,
								autoFocus: true,
								onChange: (e) => { clearFindHighlight(); setFilter(e.target.value); },
							})
							: null,
						searchOpen
							? null
