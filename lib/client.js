		...
		function apply(ctx) {
			...
			function readUi() {
				try {
					...
					if (o.open === true || o.open === false) out.open = o.open;
		...
		function previewFor(node) {
			...
			function deriveOutlineNew(snap) {
				...
				for (const turn of turnOrder) {
					...
					const g = { turn: turn, items: items, gid: "t" + turn };
					const first = items[0];
					g.title = "第 " + turn + " 轮";
					const head = (first.kind === "user" || first.kind === "input-message") ? first : null;
					g.headKey = head ? head.key : first.key;
					g.headIsUser = head !== null;
					g.subtitle = truncate((head ? head.preview : first.preview) || "", 60);
					for (const it of items) byKey[it.key] = g.gid;
					groups.push(g);
					count += items.length;
				}
				return { groups: groups, count: count, byKey: byKey };
			...
			function ToggleButton(props) {
				...
				React.useEffect(() => {
					...
					/* 会话恢复的时序竞争修复:binding/target 未物化时不再静默放弃,
					   订阅 sessions.list 在就绪后自动重试(重启后导航空的根因) */
					const trySubscribe = () => {
						if (props.sessionId == null || sessions === undefined || uiConversation === undefined) {
							setState({ outline: null, dbg: { sid: props.sessionId, sub: 2, turns: null, groups: 0 } });
						...
						const update = () => {
							try {
								...
								const turnOrder = (snap !== null && snap !== undefined && snap.timeline !== null && snap.timeline !== undefined
									&& Array.isArray(snap.timeline.turnOrder)) ? snap.timeline.turnOrder.length : null;
								setState({ outline: outline, dbg: { sid: props.sessionId, sub: 1, turns: turnOrder, groups: outline.groups.length } });
							} catch (err) {
								console.error("[cnvnav] derive failed", err);
								setState({ outline: null, dbg: { sid: props.sessionId, sub: 3, turns: null, groups: -1, err: String(err && err.message ? err.message : err) } });
							}
						};
			...
			function Panel() {
				...
				const [searchOpen, setSearchOpen] = React.useState(NEW_HOST);
				...
				React.useEffect(() => {
					...
					function place() {
						...
						/* 新宿主 dock 右侧需让出官方 rail 的 28px 竖带(right: calc(12px - clearance) + width:28px),
						   留 40px 总避让;旧宿主维持 12px */
						const railClearance = NEW_HOST ? 40 : 12;
				...
				return React.createElement("div", {
					...
					React.createElement("div", {
						...
						groups.length === 0
							? React.createElement("div", { className: "cnvnav-empty" },
								"当前会话暂无对话节点",
								snap.dbg !== undefined && snap.dbg !== null
									? React.createElement("div", { style: { marginTop: 8, fontSize: 10, color: "var(--dsw-alias-label-secondary, #999)", fontFamily: "monospace" } },
										"sid=" + (snap.dbg.sid === null || snap.dbg.sid === undefined ? "无" : "有")
										+ " · sub=" + snap.dbg.sub
										+ (snap.dbg.turns !== null && snap.dbg.turns !== undefined ? " · turns=" + snap.dbg.turns : "")
										+ " · groups=" + snap.dbg.groups
										+ (snap.dbg.err !== undefined ? " · err=" + snap.dbg.err : ""))
									: null)
