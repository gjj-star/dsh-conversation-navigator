	...
	factory: (require) => {
		...
		function PinIcon() {
			return React.createElement("svg", {
				...
				React.createElement("path", { d: "M7 1.4v1.1" }),
				React.createElement("path", { d: "M4.5 3L5.1 6.3H3.6a.9.9 0 0 0-.6 1.5L7 11.6l4-3.8a.9.9 0 0 0-.6-1.5H8.9L9.5 3" }),
				React.createElement("path", { d: "M7 11.6v1.4" }),
			);
		}
...
.cnvnav-mini-wrap-left .cnvnav-mini-fab { right: auto; left: -12.5px; }
`;
		...
		function previewFor(node) {
			...
			function Panel() {
				...
				function endDrag() {
					...
					/* 拖完同步封顶(与 place() 浮动分支一致), 超长展开后底部按钮仍可达 */
					const vh = document.documentElement.clientHeight || 800;
					d.el.style.maxHeight = Math.max(160, Math.round(vh - Math.max(8, Math.round(r.top)) - 8)) + "px";
				...
				React.useEffect(() => {
					...
					function place() {
						...
						/* 面板被拖走后不再自动停靠, 只夹进视口并封顶, 保证展开超长轮次时内部列表滚动、底部按钮可见 */
						if (!mini && !state.docked && state.userPos !== null) {
							...
							panel.style.maxHeight = Math.max(160, Math.round(vh - top - 8)) + "px";
