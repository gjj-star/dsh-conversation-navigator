...
`;
		...
		function previewFor(node) {
			...
			const KIND_META_NEW = {
				"user": { label: "你", cls: "traj-user" },
				"input-message": { label: "你", cls: "traj-user" },
				"steering": { label: "插话", cls: "traj-user" },
				"context": { label: "上下文", cls: "traj-context" },
				"assistant": { label: "助手", cls: "traj-assistant" },
				"assistant-step": { label: "助手", cls: "traj-assistant" },
				"tool-call": { label: "工具", cls: "traj-tool" },
				"tool-result": { label: "工具", cls: "traj-tool" },
				"request-prompt": { label: "请求", cls: "other-neutral" },
				"command": { label: "命令", cls: "other-command" },
				"compaction": { label: "压缩", cls: "traj-compacted" },
				"manual-compaction": { label: "压缩", cls: "traj-compacted" },
				"model-retry": { label: "重试", cls: "other-retry" },
				"turn-error": { label: "出错", cls: "other-error" },
				"turn-max-tokens": { label: "截断", cls: "other-error" },
				/* 官方新节点类型(0.1.2 引入):turn-process 回合过程折叠、turn-tail 轮尾、system-prompt 系统提示 */
				"turn-process": { label: "过程", cls: "traj-process" },
				"turn-tail": { label: "轮尾", cls: "other-neutral" },
				"system-prompt": { label: "系统提示", cls: "other-neutral" },
				"unknown-surface": { label: "其他", cls: "other-neutral" },
				"unknown": { label: "其他", cls: "other-neutral" },
			};
			function previewForNew(node) {
				...
				switch (node.kind) {
					...
					case "assistant-step": {
						const text = firstText(d.blocks);
						if (text !== null) return text;
						if (Array.isArray(d.blocks) && d.blocks.some((b) => b && (b.kind === "tool-call" || b.kind === "tool-result"))) return "调用工具";
						return null;
					}
					case "tool-result": {
						if (d.isError === true) return "执行失败";
						return d.call && typeof d.call.name === "string" ? d.call.name : "工具结果";
					}
					case "command":
						return typeof d.name === "string" ? d.name + (typeof d.args === "string" && d.args !== "" ? " " + d.args : "") : "命令";
					case "compaction":
						return typeof d.summary === "string" ? d.summary : "上下文已压缩";
					case "model-retry": {
						const n = Array.isArray(d.attempts) ? d.attempts.length : 0;
			...
			function deriveOutlineNew(snap) {
				const empty = { groups: [], count: 0, byKey: {} };
				if (snap === null || snap === undefined) return empty;
				/* 分组使用官方权威映射: timeline.turnOrder + locations.getTurn(turn) + nodes.get(key)。
				   旧实现按 legacy 节点 seq 与 turnEnds(全局事件 seq)比较,两者不同源导致
				   所有节点都判入第一轮(展开却见数百条)。 */
				const turnOrder = (snap.timeline !== null && snap.timeline !== undefined && Array.isArray(snap.timeline.turnOrder)) ? snap.timeline.turnOrder : [];
				const locations = snap.locations;
				const nodesMap = snap.nodes;
				if (locations === undefined || nodesMap === undefined) return empty;
				/* 解析官方节点自带 location 的轮次(仿 locationCoordinates:
				   turn / step 两类可解析, unresolved 返回 null) */
				function turnOfNode(node) {
					...
					if (loc.kind === "turn") return typeof loc.turn === "number" ? loc.turn : null;
					if (loc.kind === "step") {
						const t = loc.turn;
						if (t !== undefined && t !== null && typeof t === "object" && typeof t.turn === "number") return t.turn;
					}
					return null;
				}
				/* 纯 seq 归轮:轮次边界(start/end seq)来自官方 timeline.turns,
				   节点 location 即使为 unresolved 也能定位 —— 取"最大的 start.seq <= seq"。 */
				function turnBySeq(timeline, seq) {
					if (timeline === null || timeline === undefined) return null;
					const turns = timeline.turns;
					if (turns === undefined || turns === null || typeof turns.get !== "function") return null;
					let best = null;
					for (const turn of timeline.turnOrder) {
						const loc = turns.get(turn);
						if (loc === undefined || loc === null) continue;
						const startSeq = loc.start !== undefined && loc.start !== null && typeof loc.start.seq === "number" ? loc.start.seq : null;
						if (startSeq === null) continue;
						if (startSeq > seq) break; /* turnOrder 升序, 之后都更晚 */
						const endSeq = loc.end !== undefined && loc.end !== null && typeof loc.end.seq === "number" ? loc.end.seq : null;
						if (endSeq !== null && endSeq < seq) continue;
						best = turn;
					}
					return best;
				}
				...
				const mkItem = (node) => {
					...
					/* 官方节点自带 {kind, data} 形状,直接复用文本辅助函数 */
					const view = { kind: node.kind, data: node.data };
					return {
						key: String(node.key !== undefined && node.key !== null ? node.key : node.anchorSeq),
						kind: node.kind,
						label: meta.label,
						cls: meta.cls,
						preview: previewForNew(view),
						search: dialogueText(view),
						full: fullDialogueText(view),
						time: null,
						seq: typeof node.anchorSeq === "number" ? node.anchorSeq : null,
					};
				};
				/* 主通道:官方 locations 索引 (turnOrder -> key[]) */
				for (const turn of turnOrder) {
					...
					for (const key of keys) {
						...
						const item = mkItem(node);
						items.push(item);
						seen.add(item.key);
					}
					if (items.length === 0) continue;
					groups.push({ turn: turn, items: items, gid: "t" + turn });
					count += items.length;
				}
				/* 兜底:官方 locations 索引会对部分节点(例如被组装器排除在
				   order/locations 之外的 user 消息)漏挂; snap.nodes 是完整节点存储
				   (ChatNodeStore: get/values), 依节点自带 location 或 anchorSeq
				   对照轮次边界补进对应轮次, 保证大纲完整。 */
				if (typeof nodesMap.values === "function") {
					for (const node of nodesMap.values()) {
						if (node === undefined || node === null) continue;
						const itemKey = String(node.key !== undefined && node.key !== null ? node.key : node.anchorSeq);
						if (seen.has(itemKey)) continue;
						let turn = turnOfNode(node);
						if (turn === null && typeof node.anchorSeq === "number") turn = turnBySeq(snap.timeline, node.anchorSeq);
						if (turn === null) continue;
						let g = null;
						for (const cand of groups) { if (cand.turn === turn) { g = cand; break; } }
						if (g === null) {
							g = { turn: turn, items: [], gid: "t" + turn };
							groups.push(g);
						}
						g.items.push(mkItem(node));
						byKey[itemKey] = g.gid;
						seen.add(itemKey);
						count += 1;
					}
				}
				/* 汇总:轮次升序; 组内排序人类消息(你/插话)优先打头(与官方 rail 一致),
				   其余按 anchorSeq(系统提示锚定在轮次起始但不应压过用户消息) */
				groups.sort((a, b) => a.turn - b.turn);
				for (const g of groups) {
					const humanKind = (it) => it.kind === "user" || it.kind === "steering" || it.kind === "input-message";
					g.items.sort((a, b) => {
						const ha = humanKind(a) ? 0 : 1;
						const hb = humanKind(b) ? 0 : 1;
						if (ha !== hb) return ha - hb;
						return ((a.seq === null ? 0 : a.seq) - (b.seq === null ? 0 : b.seq));
					});
					const first = g.items[0];
					g.title = "第 " + g.turn + " 轮";
					const head = humanKind(first) ? first : null;
					g.headKey = head ? head.key : first.key;
					g.headIsUser = head !== null;
					g.subtitle = truncate((head ? head.preview : first.preview) || "", 60);
					for (const it of g.items) byKey[it.key] = g.gid;
